...
public class ReadLock implements Lock {
    ...
    public ReadLock(LockInfo lockInfo, Redisson redisson) {
    ...
    public boolean acquire() {
        ...
        try {
            ...
            boolean locked = rReadWriteLock.readLock().tryLock(lockInfo.getWaitTime(), lockInfo.getLeaseTime(), TimeUnit.SECONDS);
            // 抢锁成功,等待时间过长
            ...
            log.warn("[Global-Lock][INTERRUPTED] 获取读锁被中断: [{}]", lockInfo.getName(), e);
            Thread.currentThread().interrupt();
            return false;
        } catch (Exception e) {
            log.error("[Global-Lock][ERROR] 获取读锁异常: [{}]", lockInfo.getName(), e);
            return false;
        }
    }
