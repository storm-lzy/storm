...
public class FairLock implements Lock {
    ...
    public FairLock(LockInfo lockInfo, Redisson redisson) {
    ...
    public boolean acquire() {
        ...
        try {
            lock = redisson.getFairLock(lockInfo.getName());
            ...
            log.warn("[Global-Lock][INTERRUPTED] 获取公平锁被中断: [{}]", lockInfo.getName(), e);
            Thread.currentThread().interrupt();
            return false;
        } catch (Exception e) {
            log.error("[Global-Lock][ERROR] 获取公平锁异常: [{}]", lockInfo.getName(), e);
            return false;
        }
    }
